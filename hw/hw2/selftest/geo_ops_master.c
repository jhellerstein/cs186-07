...
double box_mindist_internal(BOX *box1, BOX *box2)
{
  ...
  if (strcmp(CurrentMemoryContext->name, "GiST temporary context"))
	elog(NOTICE, "mmgr not in GiST context");

  /* find nearest x side */
  if (box1->high.x < box2->low.x) { /* left */
	retval = (box2->low.x - box1->high.x)*(box2->low.x - box1->high.x);
  }
  else if (box1->low.x > box2->high.x) {/* right */
	retval = (box1->low.x - box2->high.x)*(box1->low.x - box2->high.x);
  }
  else {
	retval = 0; /* between */
  }
  ...
  /* find nearest y side */
  if (box1->high.y < box2->low.y) /* below */
    retval += (box2->low.y - box1->high.y)*(box2->low.y - box1->high.y);
  else if (box1->low.y > box2->high.y) /* above */
	retval += (box1->low.y - box2->high.y)*(box1->low.y - box2->high.y);
  /* else between */
	
  /* return sqrt(sum of distances squared) */
  return sqrt(retval);
}
